...
class FakeVideoDetectorWorkflow:
    ...
    def initialize_state(self, video_path: str, video_title: str = ""):
        """初始化工作流状态"""
        print("初始化工作流状态...")
        self.state = {
    ...
    async def transcribe_audio(self):
        ...
        try:
            ...
            # 检查音频处理日志文件是否存在
            audio_log_path = "/root/siton-tmp/projects/HMVD/data/audio_processing_log.json"
            if os.path.exists(audio_log_path):
                data = await self._async_load_json_file(audio_log_path)
                for item in data:
    ...
    async def describe_frames(self):
        ...
        try:
            ...
            # 检查处理日志文件是否存在
            processing_log_path = "/root/siton-tmp/projects/HMVD/data/processing_log2.json"
            ...
            # 检查是否已有推理结果
            frame_descriptions = []
            test_json_path = "/root/siton-tmp/projects/HMVD/data/frame_inference_results/frame_descriptions.json"
    ...
    def run_evidence_retrieval(self):
        """运行外部证据检索"""
        if not self.state.get("need_external_evidence", False):
            print("跳过外部证据检索，直接进入整合阶段")
            return
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
            future_retriever = executor.submit(
                self.agents['retriever'].analyze, retrieval_inputs
            )
            ...
            # 等待检索完成
            self.state["external_evidence"] = future_retriever.result()
            print("外部证据检索结果:")
    ...
    async def run_workflow_async(self, video_path: str, video_title: str = ""):
        """异步运行完整的工作流"""
        try:
            
            # 初始化状态
            self.initialize_state(video_path, video_title)
    ...
    def run_workflow(self, video_path: str, video_title: str = ""):
        """运行完整的工作流（同步接口）"""
        return asyncio.run(self.run_workflow_async(video_path, video_title))
...
def kickoff(video_path: str, video_title: str = ""):
    """启动检测流程"""
    workflow = FakeVideoDetectorWorkflow()
    return workflow.run_workflow(video_path, video_title)

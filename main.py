...
def process_single_video_simple(video_path: str, video_title: str = "", transcript_json: str = "", frame_caption_json: str = "") -> Dict:
    """简化版：处理单个视频（用于快速测试）"""
    ...
    try:
        start_time = time.time()
        result = kickoff(video_path, video_title, transcript_json=transcript_json, frame_caption_json=frame_caption_json)
        processing_time = time.time() - start_time
...
def main():
    """主函数 - 单视频测试（优化版）"""
    # 测试参数配置
    test_video_path = "/data/yyf/dataset/fakevideo/HMVD/_0oux8o9MJo.mp4"
    test_video_title = "Donald Trump wins 2024 presidential election"
    ...
    # 启动检测流程
    print(f"开始检测视频: {test_video_path}")
    print(f"视频标题: {test_video_title}")
    print("=" * 50)
    
    start_time = time.time()
    result = process_single_video_simple(test_video_path, test_video_title)
    ...
    if result['status'] == 'success':
        ...
        print(f"一致性分析: {detection_result.get('consistency_analysis', 'N/A')[:100]}...")
        print(f"AI检测: {detection_result.get('ai_detection', 'N/A')[:100]}...")
        print(f"冒犯性语言检测: {detection_result.get('offensive_language_detection', 'N/A')[:100]}...")
        print(f"事实检查: {detection_result.get('fact_checking', 'N/A')[:100]}...")
        print(f"最终分析: {detection_result.get('planner_analysis', 'N/A')[:100]}...")
        
        if detection_result.get("error"):
            print(f"执行过程中出现错误: {detection_result['error']}")
    else:
        print(f"检测流程执行失败: {result['error']}")
...
def parse_args():
    ...
    parser.add_argument("--single_video", type=str, default=None,
                        help="处理单个视频文件路径")
    parser.add_argument("--video_title", type=str, default="",
                        help="视频标题（用于单视频处理）")
...
if __name__ == "__main__":
    ...
    if args.single_video:
        ...
        start_time = time.time()
        result = process_single_video_simple(video_path, video_title)
        total_time = time.time() - start_time
        
        if result['status'] == 'success':
            print(f"视频处理完成，耗时: {total_time:.2f} 秒")
        else:
